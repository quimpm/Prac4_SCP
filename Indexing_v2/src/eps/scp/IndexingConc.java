...
public class IndexingConc {
    ...
    public static void main(String[] args)
    {
        ...
        for(int i = 0; i < numThreads; i++){
            ...
            threads_storage[i] =  new Thread(new partsBuildIndex(start,end,hash,sem,global_stats,percentage,printLocalLock, countThread, numThreads));
    ...
    public static class partsBuildIndex implements Runnable{
        ...
        ReentrantLock printLocalLock;
        int countThread, numThreads;


        private partsBuildIndex(int start, int end, InvertedIndexConc inverted, Semaphore sem, Statistics global_stats, int percentage, ReentrantLock printLocalLock, int countThread, int numThreads){
            this.start=start;
            this.end=end;
            this.inverted = inverted;
            this.sem=sem;
        ...
        public void run(){
            try {
                sem.acquire();
                this.inverted.BuildIndex(start, end, global_stats,percentage, printLocalLock,countThread, numThreads);
            }catch (InterruptedException ex){
                System.out.println("Error Writing File");
            }finally {
                sem.release();
