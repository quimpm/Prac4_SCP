...
public class InvertedIndexConc
{
    ...
    public void BuildIndex(int start, int end, Statistics global_stats, int percentage, ReentrantLock printLocalLock, int countThread, int numThreads, ReentrantLock lock)
    {
        FileInputStream is;
        long offset = start - 1;
        long currentOffset = start - 1;
        int car;
        String key="";
        Statistics local_statistics=new Statistics();
        int total= (end-KeySize+1)-start;
        double workDivision=(100/((float)percentage));
        double numChars=Math.round(((float)total)/workDivision);
        try {
            ...
            is.skip(start);
                ...
                {
                    global_stats.sumBytes();
                    local_statistics.sumBytes();
                    offset++;
                    ...
                    if (key.length()==KeySize) {
                        ...
                        // Sincronizamos el accesso al HashMap
                        lock.lock();
                        try{
                        ...
                        global_stats.sumOffset();
                        local_statistics.sumOffset();
                        global_stats.sumKey();
                        local_statistics.sumKey();
                    }
                    //Print Progres
                    /*synchronized (getClass()) {
                        if (((int) currentOffset % numChars) == 0) {
                            countThread++;
                            if (countThread < (numThreads - 1)) {
                                try {
                                    getClass().wait();
                                } catch (InterruptedException ex) {
                                }
                            } else {
                                global_stats.printStats((((float) currentOffset) / ((end - KeySize + 1) - start)));
                                getClass().notifyAll();
                            }
                        }
                    }*/
            ...
            printLocalLock.lock();
            try{
                local_statistics.printStats(100);
            }finally {
                printLocalLock.unlock();
            }
